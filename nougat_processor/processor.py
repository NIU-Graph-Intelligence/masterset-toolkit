...
class NougatProcessor:
    ...
    def get_marker_output_path(self, pdf_path: Path) -> Path:
        """Get the corresponding marker output path for a PDF (used in fallback mode)"""
        try:
            relative_path = pdf_path.relative_to(self.config.PDF_BASE_DIR)
        except ValueError:
            relative_path = Path(*pdf_path.parts[-3:])

        return self.config.MARKER_OUTPUT_BASE / relative_path.with_suffix('.md')

    def process_fallback_list(self):
        """Process FALLBACK_PDFS that marker also failed on.

        Skips files that:
        - Already have nougat output
        - Already have marker output (marker succeeded, no need for nougat)
        """
        ...
        for relative_path_str in fallback_paths:
            pdf_path = self.config.PDF_BASE_DIR.parent.parent / relative_path_str
            if not pdf_path.exists():
                print(f"Warning: PDF not found, skipping: {pdf_path}")
                continue

            output_path = self.get_output_path(pdf_path)
            marker_output_path = self.get_marker_output_path(pdf_path)

            if output_path.exists():
                already_by_nougat += 1
            elif marker_output_path.exists():
                already_by_marker += 1
            else:
        ...
        print(f"\n{'='*60}")
        print(f"Fallback mode - Processing marker failures with Nougat")
        print(f"{'='*60}")
        print(f"Total in list: {total}")
        print(f"Already processed by Marker: {already_by_marker} (skipped)")
