...
def update_citation_context_jsons(df, exact_lookup, normalized_titles, paper_ids, original_titles, venues, year_map, inverted_index):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Updating JSON files", unit="paper"):
        ...
        valid_json_path = reduced_path.replace(".md", ".json")
        citation_contexts_path = f"{CITATION_CONTEXTS_ROOT}/{valid_json_path}"
...
def main():
    ...
    # Re-parse references for the JSON update pass (we need the raw df path column, not references)
